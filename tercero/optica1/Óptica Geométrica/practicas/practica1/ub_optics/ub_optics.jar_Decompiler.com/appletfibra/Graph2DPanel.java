...
public class Graph2DPanel extends JPanel {
   ...
   protected void paintExtraText(Graphics2D g2d, int posicio) {
      int amplemax = false;
      FontRenderContext F = g2d.getFontRenderContext();
      int INTLIN = (int)g2d.getFont().getStringBounds("A", F).getHeight();
      int x0 = this.x0clip + INTLIN;
      ...
      case 2:
         S = this.TexteExtra;
         amplemax = (int)g2d.getFont().getStringBounds(S, F).getWidth() + 2 * INTLIN;
         x0 = this.x1clip - amplemax - 3 * INTLIN;
         y0 = this.y0clip + INTLIN;
         break;
      case 3:
      ...
      case 4:
         ...
         break;
      case 5:
         y0 = this.y0clip - INTLIN / 2;
         x0 = this.x0clip;
      }

      S = this.TexteExtra;
      g2d.drawString(S, x0, y0);
   }
   ...
   protected void paintAxes(Graphics2D g2d) {
      ...
      if (this.flag_eixx) {
         ...
         if (this.flag_splimits) {
            S = this.FormatX.format(this.FuncX.F(this.xminlabel));
            ampletext = (int)g2d.getFont().getStringBounds(S, F).getWidth();
            x = this.x0clip + (int)((double)(this.x1clip - this.x0clip) / (this.xmax - this.xmin) * (this.xminlabel - this.xmin));
            g2d.drawString(S, x - (int)((double)ampletext / 2.0D), this.y1clip + INTLIN);
            S = this.FormatX.format(this.FuncX.F(this.xmaxlabel));
            ampletext = (int)g2d.getFont().getStringBounds(S, F).getWidth();
            x = this.x0clip + (int)((double)(this.x1clip - this.x0clip) / (this.xmax - this.xmin) * (this.xmaxlabel - this.xmin));
            g2d.drawString(S, x - (int)((double)ampletext / 2.0D), this.y1clip + INTLIN);
         } else {
      ...
      if (this.flag_eixy) {
         ...
         if (this.flag_splimits) {
            Y = this.FuncY.F(this.yminlabel);
            S = this.FormatY.format(Y);
            ampletext = (int)g2d.getFont().getStringBounds(S, F).getWidth();
            alttext = (int)g2d.getFont().getStringBounds(S, F).getHeight();
            y = this.y0clip + (int)((double)(this.y1clip - this.y0clip) / (this.ymax - this.ymin) * (this.yminlabel - this.ymin));
            this.drawStringVertical(g2d, S, 0 + alttext, y + ampletext);
            Y = this.FuncY.F(this.ymaxlabel);
            S = this.FormatY.format(Y);
            ampletext = (int)g2d.getFont().getStringBounds(S, F).getWidth();
            alttext = (int)g2d.getFont().getStringBounds(S, F).getHeight();
            y = this.y0clip + (int)((double)(this.y1clip - this.y0clip) / (this.ymax - this.ymin) * (this.ymaxlabel - this.ymin));
            this.drawStringVertical(g2d, S, 0 + alttext, y);
         } else {
      ...
      if (this.flag_extra_text > 0) {
         this.paintExtraText(g2d, this.flag_extra_text);
      }
