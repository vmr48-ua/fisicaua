...
public class SliderText extends JPanel {
   ...
   public void setValmax(double myValmax) {
      if (!(myValmax <= this.valmin)) {
         int oldstep = this.slider.getMaximum();
         int newstep = (int)((double)(oldstep + 1) * (myValmax - this.valmin) / (this.valmax - this.valmin) - 1.0D + 0.5D);
         this.valmax = myValmax;
