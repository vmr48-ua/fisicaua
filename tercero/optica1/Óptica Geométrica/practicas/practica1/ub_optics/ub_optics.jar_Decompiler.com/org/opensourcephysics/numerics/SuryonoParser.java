...
public final class SuryonoParser extends MathExpParser {
   ...
   public SuryonoParser(String var1, String var2) throws ParserException {
      this(1);
      this.defineVariable(1, var2);
      this.define(var1);
      this.parse();
      if (this.getErrorCode() != 0) {
         String var3 = "Error in function string: " + var1;
   ...
   public SuryonoParser(String var1, String var2, String var3) throws ParserException {
      this(2);
      this.defineVariable(1, var2);
      this.defineVariable(2, var3);
      this.define(var1);
      this.parse();
      if (this.getErrorCode() != 0) {
         String var4 = "Error in function string: " + var1;
         var4 = var4 + '\n' + "Error: " + this.getErrorString();
         var4 = var4 + '\n' + "Position: " + this.getErrorPosition();
   ...
   public SuryonoParser(String var1, String[] var2) throws ParserException {
      ...
      for(int var3 = 0; var3 < var2.length; ++var3) {
         this.defineVariable(var3 + 1, var2[var3]);
      }
   ...
   private String removeEscapeCharacter(String var1) {
   ...
   public void defineVariable(int var1, String var2) {
      if (var1 <= this.var_count) {
         this.var_name[var1 - 1] = var2;
      }
   }

   public void setVariable(int var1, double var2) {
      if (var1 <= this.var_count) {
         this.var_value[var1 - 1] = var2;
      }
   }

   public void setVariable(String var1, double var2) {
      for(int var4 = 0; var4 < this.var_count; ++var4) {
         if (this.var_name[var4].equals(var1)) {
            this.var_value[var4] = var2;
            break;
         }
      }
   ...
   public void parse(String var1) throws ParserException {
      this.define(var1);
      this.parse();
      if (this.getErrorCode() != 0) {
         String var2 = "Error in function string: " + var1;
         var2 = var2 + '\n' + "Error: " + this.getErrorString();
         var2 = var2 + '\n' + "Position: " + this.getErrorPosition();
   ...
   public String[] parseUnknown(String var1) throws ParserException {
   ...
   public double evaluate(double var1, double var3) {
      if (this.var_count != 2) {
         ...
         this.var_value[0] = var1;
         this.var_value[1] = var3;
         return this.evaluate();
      }
   }

   public double evaluate(double var1, double var3, double var5) {
      if (this.var_count != 3) {
         return 0.0D;
      } else {
         this.var_value[0] = var1;
         this.var_value[1] = var3;
         this.var_value[2] = var5;
         return this.evaluate();
      }
   }
   ...
   public double evaluate(double[] var1) {
      if (this.var_value.length != var1.length) {
         System.out.println("JEParser Error: incorrect number of variables.");
         return 0.0D;
      } else {
         System.arraycopy(var1, 0, this.var_value, 0, var1.length);
         return this.evaluate();
      }
   }
   ...
   public void setFunction(String var1, String[] var2) throws ParserException {
      this.function = var1;
      if (var2.length != this.var_count) {
         this.var_count = var2.length;
         this.references.clear();
         this.refnames.clear();
         ...
         if (var1.equals("pi")) {
            ...
            if (this.character != '(') {
               ...
               if (this.character != ',') {
                  ...
                  this.addCode('\b');
                  String var4 = new String(this.postfix_code);
                  this.postfix_code = "";
                  this.scanAndParse();
                  if (this.character != ',') {
                     throw new ParserException(11);
                     ...
                     this.addCode('\u0001');
                     var4 = var4 + (char)(this.postfix_code.length() + 2);
                     var4 = var4 + this.postfix_code;
                     this.postfix_code = "";
                     this.scanAndParse();
                     if (this.character != ')') {
                        throw new ParserException(2);
                     } else {
                        var4 = var4 + (char)(this.postfix_code.length() + 1);
                        var4 = var4 + this.postfix_code;
                        this.postfix_code = new String(var4);
                        this.getNextCharacter();
                     }
                  }
               }
            ...
            for(var2 = 0; var2 < 26; ++var2) {
               if (var1.equals(this.funcname[var2])) {
                  this.skipSpaces();
                  if (this.character != '(') {
                     throw new ParserException(2);
                  }
            ...
            for(var2 = 0; var2 < 4; ++var2) {
               if (var1.equals(this.extfunc[var2])) {
                  this.skipSpaces();
                  if (this.character != '(') {
                     throw new ParserException(2);
                  }
            ...
            } else if (!this.appendVariables || !this.append(var1)) {
               this.position = this.start;
   ...
   private boolean append(String var1) {
      String[] var2 = new String[this.var_count + 1];
      double[] var3 = new double[this.var_count + 1];
      System.arraycopy(this.var_name, 0, var2, 0, this.var_count);
      System.arraycopy(this.var_value, 0, var3, 0, this.var_count);
      var2[this.var_count] = var1;
      this.var_name = var2;
      this.var_value = var3;
      ++this.var_count;

      for(int var4 = 0; var4 < this.var_count; ++var4) {
         if (var1.equals(this.var_name[var4])) {
            this.addCode((char)(var4 + 2000));
            return true;
         }
      }
   ...
   private double builtInFunction(int var1, double var2) {
      ...
      case 0:
         ...
         return Math.sin(var2 * 0.017453292519943295D);
      case 1:
         if (this.radian) {
            return Math.cos(var2);
         }

         return Math.cos(var2 * 0.017453292519943295D);
      case 2:
         if (this.radian) {
            return Math.tan(var2);
         }

         return Math.tan(var2 * 0.017453292519943295D);
      case 3:
         return Math.log(var2);
      case 4:
         return Math.log(var2) / LOG10;
      case 5:
         return Math.abs(var2);
      case 6:
         return Math.rint(var2);
      case 7:
         return var2 - Math.rint(var2);
      case 8:
         if (this.radian) {
            return Math.asin(var2);
         }

         return Math.asin(var2) / 0.017453292519943295D;
      case 9:
         if (this.radian) {
            return Math.acos(var2);
         }

         return Math.acos(var2) / 0.017453292519943295D;
      case 10:
         if (this.radian) {
            return Math.atan(var2);
         }

         return Math.atan(var2) / 0.017453292519943295D;
      case 11:
         return (Math.exp(var2) - Math.exp(-var2)) / 2.0D;
      case 12:
         return (Math.exp(var2) + Math.exp(-var2)) / 2.0D;
      case 13:
         double var4 = Math.exp(var2);
         double var6 = Math.exp(-var2);
         return (var4 - var6) / (var4 + var6);
      case 14:
         return Math.log(var2 + Math.sqrt(var2 * var2 + 1.0D));
      case 15:
         return Math.log(var2 + Math.sqrt(var2 * var2 - 1.0D));
      case 16:
         return Math.log((1.0D + var2) / (1.0D - var2)) / 2.0D;
      case 17:
         ...
         try {
            ...
            case '\u0001':
               ...
               while(true) {
                  ...
                  if (this.postfix_code.charAt(var2++) == 255) {
                     ++this.numberindex;
                  }
               }
            case '\u0002':
            ...
            case '/':
               ...
               break;
            case '^':
               this.stack[var1 - 1] = Math.pow(this.stack[var1 - 1], this.stack[var1]);
               --var1;
               break;
            case '_':
            ...
            case 'ý':
               ++var1;
               this.stack[var1] = 3.141592653589793D;
               break;
            case 'þ':
               ++var1;
               this.stack[var1] = 2.718281828459045D;
               break;
            case 'ÿ':
               ++var1;
            ...
            default:
               if (var4 >= 3000) {
                  ...
                  this.stack[var1 - 1] = this.builtInExtFunction(var4 - 1026, this.stack[var1 - 1], this.stack[var1]);
                  --var1;
               } else {
                  if (var4 < 1000) {
                     this.error = 8;
