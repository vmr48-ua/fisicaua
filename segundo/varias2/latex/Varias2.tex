...
\chapter{Campo de gradiente, divergencia y rotacional}
  \section{Campo escalar, campo vectorial}
    ...
    \comentario{
      Si $\mcD$ es un abierto 'simplemente conexo' de $\bbR^3$,
    ...
    \teorema{}{
      \begin{itemize}
        ...
        \item \begin{tabular}{c c c}
                $\begin{cases}
                  \text{$\mcD$ estrellado}\\[5pt]
                  \text{\ \ \ \ \ \ \ \ \ \ ó}\\[5pt]
                  \text{$\mcD$ simplemente conexo}
                \end{cases}$&
                $\Longleftrightarrow$&
                $\begin{cases}
                  \frac{\partial R}{\partial y}=\frac{\partial Q}{\partial z}\\[5pt]
                  \frac{\partial P}{\partial z}=\frac{\partial R}{\partial x}\\[5pt]
    ...
    \comentario{
      ...
      cierto, i.e. $\ovl{rot}\ovl{V}=\ovl{0}\implies\ovl{V}=\ovl{\nabla}\varphi$
...
\chapter{Integrales de línea}
  \section{Curvas paramétricas}
    ...
    \comentario{
      ...
      \end{cases}\Longleftrightarrow M_0\ovl{M}=t\ovl{T}$, entonces $\ovl{T}$ es
      tangente a $C$ en $M_0$.
    }
  ...
  \section{Longitud de una curva}
    ...
    \ejercicio{Halla la longitud de arco de la siguiente curva: $\gamma\left(t\right)
    =\setlength\extrarowheight{0pt}\begin{cases}\left(\cos(t),\sin(t),3t\right)\text{
...
\chapter{Integrales dobles}
  ...
  \section{Teoremas de Fubini}
    ...
    \teorema{Fubini versión fuerte}{
      \begin{itemize}
        \item Sean $\varphi_1$ y $\varphi_2$ dos funicones continuas en un rectángulo
        ...
        \item Sean $\psi_1$ y $\psi_2$ dos funicones continuas en un rectángulo
    ...
    \right)\in\bbR^2\colon 0\leq \leq1, x^2\leq y\leq x\right\}$ (no es un rectángulo),
    sobre la función $f\left(x,y\right)=x+y$}{
      ...
      \end{wrapfigure}
      \vspace{0.6cm}
      $\iint_\mcD f\left(x,y\right)dxdy = \int_{0}^{1}\int_{x^2}^{x}\left(x,y\right)
    ...
    \subsection{Introducción}
      ...
      \ejemplo{Sea $\Phi\colon\bbR^2\rightarrow\bbR^3$ dada por
        $\begin{cases}
          ...
          y=u\sin v\\
          z=u^2+v^2
        \end{cases}$ \\¿Existe un plano tangente? Hallar el plano tangente en $\Phi\left(0,1\right)$
        }{
    ...
    \subsection{Área de una superficie}
      ...
      \ejemplo{
        ...
        $\begin{cases}
          ...
          y=r\sin\theta\\
          z=r
        \end{cases}$ hallar su área de superficie.
      }{
        $\frac{\partial\left(x,y\right)}{\partial\left(r,\theta\right)}=
        \begin{vmatrix}
          \cos\theta&   -r\sin\theta\\
          \sin\theta&    r\cos\theta
        \end{vmatrix}$\hspace{0.5cm}$\frac{\partial\left(y,z\right)}{\partial\left(r,\theta\right)}
        ...
        +sin^2\theta}=\sqrt{2}r$\\ \\$\mcA=\int_0^2\pi\int_0^1\sqrt{2}r\ dr\ d\theta =\sqrt{2}\pi$
      }
      \comentario{
        ...
        $\begin{cases}
          r=r'\\
          \theta=\theta'
        \end{cases}$ y eso nos dice que $\Phi$ es biyectiva.\\
      }
      \ejemplo{
        ...
        $\begin{cases}
          ...
          y=r\sin\theta\\
          z=\theta
        \end{cases}$ y $\mcD$ es la región donde $0\leq\theta\leq 2\pi$ y
      ...
      \comentario{
        ...
        \end{cases}$ con $\left(u,v\right)\in\mcD$. \\ \\
  ...
  \section{Integrales de funciones escalares sobre superficies}
    ...
    \comentario{
      Si $z=g\left(x,y\right)$ con $g\in\mcC_1$ tenemos:
    ...
    \ejemplo{Sea $\mcS$ una superficie definida por $z=x^2+y$ donde $\mcD$ es la region
    caracterizada por $0\leq x\leq 1$ y $-1\leq y\leq 1$. Calcular $\iint_\mcD x\ d\mcS$}{
      Tenemos $\iint_\mcS x\ d\mcS=\iint_\mcD x\sqrt{1+4x^2+1}\ dx\ dy=\int_{-1}^{1}\left(
      ...
      \left[\left(2+4x^2\right)^\frac{3}{2}\right]_0^1\ dy=\frac{1}{12}\int_{-1}^{1}\left(
    ...
    \comentario{
      ...
      \begin{wrapfigure}{l}{.5\textwidth}
        \vspace{-0.5cm}
        \centering
        \includegraphics[width=.5\textwidth]{mireia2.png}
      \end{wrapfigure}    
      $\Phi\left(x,y,z\right)=z-g\left(x,y\right)=0\text{ un vector normal a }
      \Phi\text{ es }\ovl{n}=\ovl{\nabla}\Phi\text{, es decir, }\boxed{\ovl{n}=
      -\frac{\partial g}{\partial x}\ovl{i}-\frac{\partial g}{\partial y}\ovl{j}+\ovl{k}}
      \text{ y tenemos }\ovl{n},\ovl{k}=\norm{\ovl{n}}\cdot\norm{\ovl{k}}\cdot
      \cos\left(\ovl{n},\ovl{k}\right) \Longleftrightarrow\cos\theta=
      \frac{\ovl{n}\cdot\ovl{k}}{\norm{\ovl{n}}\cdot\norm{ovl{k}}}=
      \frac{\ovl{n}\cdot\ovl{k}}{\norm{\ovl{n}}}=\frac{1}{\norm{\ovl{n}}} 
      \Longleftrightarrow\norm{\ovl{n}}=\frac{1}{\cos\theta}
      \text{ (sabiendo que }\ovl{k}\text{ es unitario y es }\left(0,0,1\right)\text{)}
      \text{\\Al final obtenemos,}\boxed{\iint_\mcS f\ d\mcS=\iint_\mcD\frac{f\left(x,y,
      g\left(x,y\right)\right)}{\cos\theta}}\ dx\ dy$
    }
    ...
    vector normal $\ovl{n}$ unitario, tendriámos $\ovl{n}\cdot\ovl{k}=\cos\theta$)}{
      ...
      este plano tine coordenadas $\left(1,1,1\right)$. Tenemos $\norm{\ovl{n}}=\sqrt{3}$
      ...
      {x}{\ovl{n}\cdot\ovl{k}}\ dx\ dy=\iint_\mcD \frac{x}{\frac{1}{\sqrt{3}}}$ donde 
      $\ovl{n}\cdot\ovl{k}=\norm{\ovl{n}}\norm{\ovl{k}}\cos\theta\Leftrightarrow
      \ovl{n}\cdot\ovl{k}=\cos\theta$ porque $\norm{\ovl{n}}\norm{\ovl{k}}=1$ por unitarios.\\
      $\iint_\mcD \frac{x}{\frac{1}{\sqrt{3}}}=\sqrt{3}\int_{0}^{1}\int_{0}^{1-x}x\ dy\ dx=
      \frac{\sqrt{3}}{6}$      
    }
