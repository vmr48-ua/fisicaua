...
plt.title("Pressió de vapor en funció de la temperatura", fontsize = 15)
...
#plt.plot(x,np.polyval(P,x),label='Linealización',color = 'r')
plt.legend(loc='best')
plt.grid()
...
print('R² =',round(ajuste.rvalue,4))
plt.savefig('Presión de vapor.png')

plt.figure(figsize=(9,6),dpi=250)
plt.title("Pressió de vapor en funció de la temperatura corregida", fontsize = 15)
x = np.linspace(temp[0],temp[-1])
...
plt.xlabel('Temperatura $(k)$')
plt.ylabel('Pressió $(Pa)$')
plt.savefig('Presión de vapor corregida.png')
...
plt.title("Equació de Clausius-Clapeyron", fontsize = 15)
plt.xlabel('$T^{-1}$ $(K^{-1})$')
plt.ylabel("$ \\log(\\frac{P_v}{P_0})$")
label = "$ \\log\\left(\\frac{P_v}{P_0}\\right) = \\left(-5800 \\pm 200\\right)"+
"\\frac{1}{T} + \\left(15,3 \\pm 0,6\\right) $ \n $ R^2 = 0.9909"
...
plt.errorbar(temp,P_v,yerr=errpv, fmt='o',label='Dades experimentals',
ms=4,marker='x',elinewidth = 1, capsize = 4, capthick = 1,)
plt.plot(t,Antoine(t),"g",label = "Aproximació de Antoine")
...
plt.figure()
plt.title("Comparativa d'entalpies de vaporització", fontsize = 14)
...
plt.figure()
plt.title("Comparativa de temperatures d'ebullició", fontsize = 14)
